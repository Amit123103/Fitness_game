...
import { saveUserToMemory, getUserFromMemory, type UserRecord } from '../services/userStore.js';
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    let user: UserRecord | undefined = getUserFromMemory(email);
...
export const supabaseLogin = async (req: Request, res: Response) => {
  ...
  const targetEmail: string = email || 'test@example.com';
  const targetUid: string = supabaseUid || 'test-user-123';

  try {
    let user: UserRecord | undefined = getUserFromMemory(targetEmail) || getUserFromMemory(targetUid);
